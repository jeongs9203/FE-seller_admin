...
export default function FileUploadImageProductDetail({
  label = 'Upload Files',
  btnLabel = 'Upload',
  fieldLabel,
  multiple = true,
  ...
  setDetailImage?: React.Dispatch<React.SetStateAction<productCreateImageType[]>>;
}) {
...
export const FileInput = ({
  ...
  function handleImageDelete(index: number) {
    ...
    setDetailImage && setDetailImage(
      (prevDetailImage) => prevDetailImage.filter((_, i) => i !== index)
    );
    (imageRef.current as HTMLInputElement).value = '';
  }

  const handleFileUpload = async () => {
    if (files.length && setDetailImage) {
